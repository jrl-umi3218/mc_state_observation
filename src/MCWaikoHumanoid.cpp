...
#include <mc_state_observation/MCWaikoHumanoid.h>
...
MCWaikoHumanoid::MCWaikoHumanoid(const std::string & type, double dt, bool asBackup)
: mc_observers::Observer(type, dt), estimator_(dt, alpha_, beta_, 1 / (2 * M_PI), 1), odometryManager_(dt)
{
  asBackup_ = asBackup;
}

void MCWaikoHumanoid::configure(const mc_control::MCController & ctl, const mc_rtc::Configuration & config)
{
...
void MCWaikoHumanoid::reset(const mc_control::MCController & ctl)
{
  ...
  estimator_.pushInput(stateObservation::InputWaikoHumanoid());
...
bool MCWaikoHumanoid::run(const mc_control::MCController & ctl)
{
  const auto & realRobot = ctl.realRobot(robot_);
...
void MCWaikoHumanoid::updateNecessaryFramesOdom(const mc_control::MCController & ctl, const mc_rbdyn::Robot & odomRobot)
...
void MCWaikoHumanoid::runTiltEstimator(const mc_control::MCController & ctl, const mc_rbdyn::Robot & odomRobot)
{
...
void MCWaikoHumanoid::updatePoseAndVel(const so::Vector3 & localWorldImuLinVel, const so::Vector3 & localWorldImuAngVel)
{
...
void MCWaikoHumanoid::update(mc_control::MCController & ctl)
{
...
void MCWaikoHumanoid::update(mc_rbdyn::Robot & robot)
{
  robot.posW(poseW_);
  robot.velW(velW_);
}

const so::kine::Kinematics MCWaikoHumanoid::backupFb(boost::circular_buffer<so::kine::Kinematics> * koBackupFbKinematics)
{
...
// void MCWaikoHumanoid::delayedOriMeasurementHandler(const mc_control::MCController & ctl,
...
void MCWaikoHumanoid::setOdometryType(OdometryType newOdometryType)
{
...
void MCWaikoHumanoid::addDelayedOriMeasLogs(mc_rtc::Logger & logger, const std::string & category)
{
...
void MCWaikoHumanoid::removeDelayedOriMeasLogs(mc_rtc::Logger & logger)
{
...
void MCWaikoHumanoid::addToLogger(const mc_control::MCController & ctl,
...
void MCWaikoHumanoid::removeFromLogger(mc_rtc::Logger & logger, const std::string & category)
{
...
void MCWaikoHumanoid::addToGUI(const mc_control::MCController &,
...
EXPORT_OBSERVER_MODULE("MCWaikoHumanoid", mc_state_observation::MCWaikoHumanoid)
