...
struct MCWaikoHumanoid : public mc_observers::Observer
{
...
public:
  /// @brief Constructor for the MCWaikoHumanoid.
  /// @details The parameters asBackup is given only if WaikoHumanoid is used as a backup by the
  /// Kinetics Observer
  MCWaikoHumanoid(const std::string & type, double dt, bool asBackup = false);
